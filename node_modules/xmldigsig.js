exports.Validator = (function() {
	...
	var dcrypt;
	try{
		dcrypt = require('webinos-certificateHandler').dcrypt();
	} catch(e) { }
	...
	Validator.prototype.parse = function(sigBuffer) {
		...
		var endElement = function(elt) {
			...
			if(isSig && name == 'DigestValue') {
				...
				var x509Cert;
				try {
					x509Cert = dcrypt.parseCert(elt.text, 'base64');
					if(x509Cert) {
				...
				try {
					x509CRL = dcrypt.parseCrl(elt.text, 'base64');
					if(x509CRL) {
	...
	Validator.prototype.validateSignature = function() {
		...
		if(this.SignatureMethod == SIG_ALGORITHM_RSAwithSHA256) {
			verify = dcrypt.createVerify('RSA-SHA256');
		} else if(this.SignatureMethod == SIG_ALGORITHM_DSAwithSHA1) {
			verify = dcrypt.createVerify('DSA-SHA1');
		} else {
		...
		var result = verify.verify(this.certificatePath[0].public_key, this.SignatureValue, 'base64');
		return result;
	};
